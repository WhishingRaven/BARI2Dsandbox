...
macOS에서 직접 실행하면 키보드 포커스를 명시적으로 잡을 수 있는 Tk 창으로 연다. 창을 연 직후 또는 버튼을 누른 뒤 키가 먹지 않는 것처럼 보이면 **왼쪽 지도 영역을 한 번 클릭**하면 조종 입력으로 포커스가 돌아온다. 한글 두벌식 입력 상태여도 W/S/A/D/Z/C/E/Q/R에 대응하는 ㅈ/ㄴ/ㅁ/ㅇ/ㅋ/ㅊ/ㄷ/ㅂ/ㄱ 입력을 같은 조종키로 처리한다.
